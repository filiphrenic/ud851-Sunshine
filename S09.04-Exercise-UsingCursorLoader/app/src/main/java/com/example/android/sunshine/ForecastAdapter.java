...
//      COMPLETED (7) Generate a weather summary with the date, description, high and low
        ...
        String description = SunshineWeatherUtils.getStringForWeatherCondition(
                mContext,
                mCursor.getInt(weatherIDIndex)
        );

        double maxTemp = mCursor.getDouble(maxTempIndex);
        double minTemp = mCursor.getDouble(minTempIndex);

        String temperature = SunshineWeatherUtils.formatHighLows(mContext, maxTemp, minTemp);

        String weatherSummary = date + " - " + description + " - " + temperature;
...
//      COMPLETED (12) After the new Cursor is set, call notifyDataSetChanged
    public void swapCursor(Cursor cursor) {
        ...
        mCursor = cursor;
        if (mCursor != null) {
            dateIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_DATE);
            weatherIDIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_WEATHER_ID);
            maxTempIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MAX_TEMP);
            minTempIndex = mCursor.getColumnIndex(WeatherContract.WeatherEntry.COLUMN_MIN_TEMP);
        }
